...
function CategoriesPage() {
    ...
    const fetchNewsByCategory = async (page) => {
        try {
            const response = await axios.get(`http://localhost:5000/api/published-news?categories=${selectedCategory}`, {
                params: {
                    page,
                    limit: 10
                }
                ...
                <div className="grid grid-cols-1 gap-6">
                    {news.map((item) => (
                        <div key={item._id} className="bg-white p-4 rounded-lg shadow-md">
                            <h2 className="text-xl font-bold mb-2 text-sky-500"><a href={`/Admin/edit/${item._id}`}>{item.title}</a></h2>
                            <p>{item.content}</p>
